...
def _write_golden_snapshot(res, *, weights_hash: str, config_hash: str) -> None:
    """Materialize the deterministic golden portfolio for human/agent review.

    The hash remains the hard CI gate. These files are an inspectable companion
    generated before the assertion so future failures still produce artifacts.
    """
    table = _golden_weights_table(res)
    table.to_csv(GOLDEN_CSV_FILE, index=False)

    last = res.weights.index[-1]
    last_date = str(last.date() if hasattr(last, "date") else last)
    total_weight = float(table["weight"].sum()) if len(table) else 0.0
    rows = [
        "# Golden weights snapshot",
        "",
        "Deterministic synthetic portfolio generated by `tests/test_regression.py`.",
        "This is not a live-capital recommendation; it is the CI behavior contract",
        "for the pinned synthetic provider/configuration.",
        "",
        f"- weights_hash: `{weights_hash}`",
        ...
        f"- synthetic_portfolio_date: `{last_date}`",
        f"- position_count: `{len(table)}`",
        f"- total_weight: `{total_weight:.8f}`",
        "",
...
def test_golden_weights():
    ...
    _write_golden_snapshot(res, weights_hash=h, config_hash=config_hash)
