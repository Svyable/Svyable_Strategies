...
def test_alpha_factor_table_includes_both_new_alpha_families():
    ...
    assert {"Alpha Catalyst", "Tape Acceleration"}.issubset(set(table["family"]))
...
def test_alpha_dashboard_metrics_are_nonzero():
    ...
    assert metrics["alpha_factor_count"] == len(ALPHA_CATALYST) + len(TAPE_ACCELERATION)
    assert metrics["alpha_strategy_count"] == 2
    assert metrics["shadow_factors"] >= len(ALPHA_CATALYST)


if __name__ == "__main__":
    test_alpha_factor_table_includes_both_new_alpha_families()
