"""Run ledger — the observability spine (local-first).

One SQLite file records every run, order plan, daily equity point, and
operational event. Everything queryable; the dashboard, the Claude loop, and
the drift monitor all read from here instead of scraping CSV directories.
"""
...
CREATE TABLE IF NOT EXISTS runs (
  ...
  ts TEXT NOT NULL,
  kind TEXT NOT NULL,               -- daily | backtest | rebalance | walkforward
...
CREATE TABLE IF NOT EXISTS equity (
  d TEXT PRIMARY KEY,               -- date
  shadow_nav REAL,                  -- cumulative NAV of the shadow backtest book
  paper_equity REAL,                -- broker/paper account equity
  drift_bps REAL,                   -- daily live-vs-shadow return gap, bps
  note TEXT
);
...
class Ledger:
        ...
        for o in planned:
            self.con.execute(
                ...
                (datetime.now().isoformat(timespec="seconds"), run_id, broker,
                 int(dry_run), o["symbol"], o["side"], o["qty"], o["est_price"],
                 o["est_notional"],
                 "planned" if dry_run else status_by_symbol.get(o["symbol"], "submitted"),
                 o.get("reason", "")))
        self.con.commit()

    def record_equity(self, d: date | str, *, shadow_nav: float | None = None,
                      paper_equity: float | None = None, note: str = "") -> None:
        """Upsert; drift computed once both series have a prior point."""
        d = str(d)
        row = self.con.execute("SELECT shadow_nav, paper_equity FROM equity WHERE d=?",
                               (d,)).fetchone()
        if row:
    ...
    def record_event(self, level: str, source: str, message: str) -> None:
        self.con.execute("INSERT INTO events (ts, level, source, message) VALUES (?,?,?,?)",
    ...
    def health(self) -> dict:
        """One-call status for the Claude loop / dashboard."""
        ...
        return {
            ...
            "drift_within_30bps_frac": round(float((drift.abs() <= 30).mean()), 3) if len(drift) else None,
            "warnings_7d": int(warn), "critical_7d": int(crit),
        }
