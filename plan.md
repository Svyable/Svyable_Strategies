...
4. **Phase 3 — Paper execution.** Build `BrokerConnector` + `Order`/`Fill`/`Position`/`Account` models; stand up the `LocalPaperBroker` and the tastytrade sandbox adapter as the path to an end-to-end paper-trade integration test; build `execution/rebalancer.py` (target weights → orders) and a reconciliation view added to the dashboard.
...
- New package: `src/q23/execution/` — `broker.py` (protocol + models), `rebalancer.py`, `adapters/{tastytrade,schwab,ibkr}.py`.
